...
def fused_moe(
    ...
    return fused_moe_(
        ...
        dtype=dtype,
        q_dtype_a=q_dtype_a,
        fake_topk_slots=fake_topk_slots,
        hidden_pad=hidden_pad,
        intermediate_pad=intermediate_pad,
        bias1=bias1,
        bias2=bias2,
...
def get_2stage_cfgs(
    ...
    cu_num = get_cu_num()
    gfx = get_gfx_runtime()
    # Some EP clients append always-masked routing columns for shared/fake
    # experts, while MORI and SGLang DeepEP keep the real routed top-k width.
    # Never infer this from expert_mask/is_ep: the expert-id mask dimension and
    # the routing-column dimension are independent.  Callers that truly append
    # fake columns must state the exact count.
    fake_topk_slots = int(fake_topk_slots)
    if fake_topk_slots < 0 or fake_topk_slots >= topk:
        raise ValueError(
            f"fake_topk_slots must be in [0, topk), got {fake_topk_slots=} {topk=}"
        )
    topk -= fake_topk_slots
    keys = (
        gfx,
        cu_num,
        token,
    ...
    if use_mxfp4_flydsl:
        ...
        # gfx942 native A8W4 is a small-M decode path.  Direct MI308X
        # measurements on the K3 EP16-local shape show M16 is materially
        # faster than M32, with N256 for gate/up and N128 for down projection.
        if _a_type == "fp8" and _w_type == "fp4" and get_gfx() == "gfx942":
            _tile_m, _s1_sfx, _s2_sfx = 16, "_w3_bnt0", ""
            _s1_tn = pick_flydsl_stage1_tile_n(inter_dim)
            _s2_tn = 128
