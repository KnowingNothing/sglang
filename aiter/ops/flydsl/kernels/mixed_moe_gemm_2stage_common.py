            ...
            def moe_gemm1_body(shared_b: bool = False):
                ...
                def act_vec4(gate_v4, up_v4):
                    ...
                    if const_expr(shared_b and need_fp8):
                        ...
                        gate_v4 = arith.extf(
                            vec4_f32, arith.trunc_f(vec4_bf16, gate_v4)
                        )
                        up_v4 = arith.extf(vec4_f32, arith.trunc_f(vec4_bf16, up_v4))
                ...
                def act_elem(g, u):
                    ...
                    if const_expr(shared_b and need_fp8):
                        g = arith.extf(f32, arith.trunc_f(T.bf16, g))
                        u = arith.extf(f32, arith.trunc_f(T.bf16, u))
                    if const_expr(act == "swiglu"):
                        alpha = arith.constant(1.702, type=f32)
                        one = arith.constant(1.0, type=f32)
                        neg_log2e = arith.constant(-1.4426950408889634, type=f32)
...
def compile_mixed_moe_gemm1_a16w4(
    *,
    ...
    """Software MXFP4 stage1 for BF16 or gfx942 FP8 activations."""
    is_a16w4_stage1 = True
    is_a8w4_gfx942 = a_dtype == "fp8"
    if a_dtype not in ("bf16", "fp8"):
        raise ValueError(f"software MXFP4 stage1 requires bf16/fp8 A, got {a_dtype!r}")
    if is_a8w4_gfx942 and get_hip_arch() != "gfx942":
        raise ValueError("software A8W4 stage1 is specific to gfx942")
    ...
    is_f16_a = (not is_a8w4_gfx942) and (is_a16w4_stage1 or a_dtype == "fp16")
    is_f16_b = (not is_a16w4_stage1) and (b_dtype == "fp16")
    is_f8_a = is_a8w4_gfx942 or ((not is_a16w4_stage1) and a_dtype == "fp8")
    ...
    elem_bytes = 1 if is_a8w4_gfx942 else 2
    ...
    # ---- gfx942 FP8 or BF16 K32 MFMA helper ----
    mfma_f32_bf16_k32 = None
    mfma_f32_fp8_k32 = None
    if is_a16w4_stage1:
        if out_dtype not in ("f16", "bf16"):
            raise ValueError(f"out_dtype must be 'f16' or 'bf16', got {out_dtype!r}")
        if is_a8w4_gfx942:
            mfma_f32_fp8_k32 = rocdl.mfma_f32_16x16x32_fp8_fp8
        else:
    ...
    def _x_elem_type():
        if is_a16w4_stage1:
            return default_f8_type() if is_a8w4_gfx942 else T.bf16
        if is_f4_b:
            return default_f8_type() if is_f8_a else T.i8
    ...
    def moe_gemm1(
        ...
        if const_expr(is_a16w4_stage1):
            if const_expr(not is_a8w4_gfx942):
                _ = arg_scale_x
            ...
            x_elem = default_f8_type() if is_a8w4_gfx942 else T.bf16
            w_elem = T.i8
            f32 = T.f32
            i32 = T.i32
            i64 = T.i64
            ...
            vec16_x = T.vec(16 // elem_bytes, x_elem)
            vec2_i64 = T.vec(2, i64)
            ...
            with _if_then(_if_blk):
                ...
                sx_rsrc = 1
                if const_expr(is_a8w4_gfx942):
                    sorted_m = size_expert_ids_in * arith.index(tile_m)
                    sx_nbytes_idx = sorted_m * arith.index(scale_kblk_padded_s1)
                    sx_rsrc = ptr_buffer_resource(
                        arg_scale_x, arith.index_cast(i32, sx_nbytes_idx)
                    )
                ...
                def load_x_tile(base_k):
                    """gfx942 fallback: global -> VGPR -> LDS.

                    gfx942 LLVM cannot lower ``raw.ptr.buffer.load.lds`` for
                    this dynamic resource descriptor.  Keep gfx950's direct
                    DMA path, but use ordinary buffer loads on gfx942.
                    """
                    base_k_div4 = (
                    ...
                    for i in range_constexpr(num_x_loads):
                        ...
                        x_vec = buffer_copy_gmem16_dwordx4(
                            buffer_ops,
                            vector,
                            elem_type=x_elem,
                            # ``idx_i32`` is already a dword index.  The
                            # helper loads through dtype=i32, whose buffer op
                            # performs the sole x4 dword-to-byte conversion.
                            idx_i32=idx_i32,
                            rsrc=x_rsrc,
                            vec_elems=x_vec_elems,
                            elem_bytes=elem_bytes,
                        )
                        parts.append(vector.bitcast(T.vec(chunk_i32, i32), x_vec))
                    return parts
                ...
                def prefetch_x_to_lds(base_k, lds_buffer):
                    if const_expr(gpu_arch == "gfx942"):
                        store_x_tile_to_lds(load_x_tile(base_k), lds_buffer)
                    else:
                        dma_x_tile_to_lds(base_k, lds_buffer)
                ...
                _a_sublane_stride = 32 * elem_bytes
                _a_ku_stride_bytes = 8 * elem_bytes
                _a_k0_stride_bytes = 128 * elem_bytes
                ...
                def _sorted_scale_byte_index(sorted_row, scale_group):
                    """Mirror ``mx_scale_shuffle_idx`` from mx_quant_utils.h."""
                    scale_n_pad = arith.index(scale_kblk_padded_s1)
                    return (
                        (sorted_row // arith.index(32) * scale_n_pad)
                        * arith.index(32)
                        + (scale_group // arith.index(8)) * arith.index(256)
                        + (scale_group % arith.index(4)) * arith.index(64)
                        + (sorted_row % arith.index(16)) * arith.index(4)
                        + ((scale_group % arith.index(8)) // arith.index(4))
                        * arith.index(2)
                        + (sorted_row % arith.index(32)) // arith.index(16)
                    )

                def load_a_scales(base_k, ku_limit=k_unroll):
                    if const_expr(not is_a8w4_gfx942):
                        return None
                    scales = []
                    for ku in range_constexpr(ku_limit):
                        ...
                        for mi in range_constexpr(m_repeat):
                            ...
                            token_i32 = fused_i & mask24
                            slot_i32 = arith.shrui(
                                fused_i, arith.constant(24, type=i32)
                            )
                            token_valid = arith.cmpi(
                                arith.CmpIPredicate.ult, token_i32, tokens_i32
                            )
                            slot_valid = arith.cmpi(
                                arith.CmpIPredicate.ult, slot_i32, topk_i32
                            )
                            row_valid = arith.andi(token_valid, slot_valid)
                            scale_i8 = buffer_ops.buffer_load(
                                sx_rsrc,
                                _sorted_scale_byte_index(sorted_row, group),
                                vec_width=1,
                                dtype=T.i8,
                            )
                            scale_u32 = arith.select(
                                row_valid,
                                arith.extui(T.i32, scale_i8),
                                arith.constant(0, type=i32),
                            )
                            per_m.append(
                                arith.bitcast(
                                    f32,
                                    arith.shli(
                                        scale_u32, arith.constant(23, type=i32)
                                    ),
                                )
                            )
                        scales.append(per_m)
                ...
                def load_all_b_raw(base_k, k0_limit=_k0_count, ku_limit=k_unroll):
                    ...
                    return (
                        ...
                        u_v4,
                        u_scales,
                        load_a_scales(base_k, ku_limit=ku_limit),
                    )
                ...
                def _a_col_bytes_for_ku(ku_val):
                    ...
                    return col_offset_base_bytes + arith.index(
                        _ku_in * _a_ku_stride_bytes
                        + _k0_blk * _a_k0_stride_bytes
                    )
                ...
                def _wave_max(value):
                    """Return one scale shared by every lane of the MFMA wave.

                    A scales vary with the output row and B scales vary with
                    the output column.  An MFMA result fragment mixes both, so
                    a base reduced only across the four K-lane quarters cannot
                    be applied to the whole fragment afterwards.  A wave-wide
                    base is uniform for every output element and therefore
                    makes the deferred ``a_base * b_base`` multiplication
                    mathematically valid.
                    """
                    value = fx.Float32(value)
                    for offset in (1, 2, 4, 8, 16, 32):
                        value = value.maximumf(value.shuffle_xor(offset, 64))
                    return value

                def _rescale_fp8_operand(packed_i64, factor):
                    vec1_i64 = vector.from_elements(T.vec(1, i64), [packed_i64])
                    words = vector.bitcast(T.i32x2, vec1_i64)
                    out_words = []
                    for word_idx in range_constexpr(2):
                        word = vector.extract(
                            words, static_position=[word_idx], dynamic_position=[]
                        )
                        lo = fx.Vector(
                            fx.rocdl.cvt_pk_f32_fp8(T.f32x2, word, False)
                        )
                        hi = fx.Vector(
                            fx.rocdl.cvt_pk_f32_fp8(T.f32x2, word, True)
                        )
                        values = lo.shuffle(hi, [0, 1, 2, 3])
                        scaled = []
                        for elem in range_constexpr(4):
                            value = fx.Float32(values[elem]) * 0.5 * factor
                            # gfx942 FNUZ encodes 0x80 as NaN, so prevent
                            # negative underflow from rounding to negative zero.
                            is_tiny_negative = (value < 0.0) & (
                                value > -(2.0**-8)
                            )
                            scaled.append(
                                is_tiny_negative.select(fx.Float32(0.0), value)
                            )
                        packed_word = fx.Int32(0).ir_value()
                        packed_word = fx.rocdl.cvt_pk_fp8_f32(
                            i32,
                            (scaled[0] * 2.0).ir_value(),
                            (scaled[1] * 2.0).ir_value(),
                            packed_word,
                            0,
                        )
                        packed_word = fx.rocdl.cvt_pk_fp8_f32(
                            i32,
                            (scaled[2] * 2.0).ir_value(),
                            (scaled[3] * 2.0).ir_value(),
                            packed_word,
                            1,
                        )
                        out_words.append(packed_word)
                    packed = vector.bitcast(
                        T.vec(1, i64), vector.from_elements(T.i32x2, out_words)
                    )
                    return vector.extract(
                        packed, static_position=[0], dynamic_position=[]
                    )

                def _acc_scaled_f32(acc_value, partial, scale):
                    from flydsl._mlir.dialects._math_ops_gen import fma as _math_fma

                    scale_vec = arith._to_raw(
                        vector.broadcast(vec4_f32, arith._to_raw(scale))
                    )
                    return arith.ArithValue(
                        _math_fma(
                            scale_vec,
                            arith._to_raw(partial),
                            arith._to_raw(acc_value),
                        )
                    )

                def _mfma_k32(
                    acc_in, a0, a1, b0, b1, a_scale=None, b_scale=None
                ):
                    if const_expr(is_a8w4_gfx942):
                        a_base = _wave_max(a_scale)
                        b_base = _wave_max(b_scale)
                        a_factor = a_scale * rocdl.rcp(f32, a_base)
                        b_factor = b_scale * rocdl.rcp(f32, b_base)
                        a_fp8 = _rescale_fp8_operand(a0, a_factor)
                        b_fp8 = _rescale_fp8_operand(b0, b_factor)
                        partial = mfma_f32_fp8_k32(
                            vec4_f32, [a_fp8, b_fp8, acc_init, 0, 0, 0]
                        )
                        return _acc_scaled_f32(
                            acc_in, partial, a_base * b_base
                        )
                ...
                for pair_i in range_constexpr(pair_iters):
                    ...
                    (
                        ...
                        u_sc_pong,
                        a_sc_pong,
                    ) = load_all_b_raw(_k_b1 + _wg_k_off)
                    ...
                    (
                        ...
                        u_sc_ping,
                        a_sc_ping,
                    ) = load_all_b_raw(_k_b2 + _wg_k_off)
                ...
                if const_expr(total_tiles == 1):
                    ...
                    if const_expr(_pad_ku_skip > 0):
                        ...
                        (
                            ...
                            u_sc_pong,
                            a_sc_pong,
                        ) = load_all_b_raw(k_tail1 + _wg_k_off)
...
def compile_mixed_moe_gemm2_a16w4(
    *,
    ...
    """Software MXFP4 stage2 for BF16 or gfx942 FP8 activations."""
    is_a16w4 = True
    is_a8w4_gfx942 = a_dtype == "fp8"
    if a_dtype not in ("bf16", "fp8"):
        raise ValueError(f"software MXFP4 stage2 requires bf16/fp8 A, got {a_dtype!r}")
    if is_a8w4_gfx942 and get_hip_arch() != "gfx942":
        raise ValueError("software A8W4 stage2 is specific to gfx942")
    ...
    is_f16_a = (not is_a8w4_gfx942) and (is_a16w4 or (a_dtype == "fp16"))
    is_f16_b = (not is_a16w4) and (b_dtype == "fp16")
    is_f8_a = is_a8w4_gfx942 or ((not is_a16w4) and (a_dtype == "fp8"))
    ...
    elem_bytes = 1 if is_a8w4_gfx942 else (2 if is_a16w4 else 1)
    a_elem_bytes = elem_bytes if is_a16w4 else (2 if is_f16_a else 1)
    b_elem_bytes = 1
    tile_k_bytes = int(tile_k) * int(a_elem_bytes)
    ...
    # Software MXFP4 path: BF16 MFMA or gfx942 FP8 MFMA.
    mfma_f32_bf16_k32 = None
    mfma_f32_fp8_k32 = None
    ...
    module_name = (
        ...
        f"{'_gfx942_a8w4_abi4' if is_a8w4_gfx942 else ''}"
    ).replace("-", "_")
    ...
    _input_elems = _single_x_bytes if a_elem_bytes == 1 else (_single_x_bytes // 2)
    _single_x_elems = (
        _single_x_bytes // int(a_elem_bytes) if is_a16w4 else _input_elems
    )
    ...
    if True:
        ...
        def moe_gemm2(
            ...
            # FP8 activation scales use the sorted-row tiled E8M0 layout.
            sx_rsrc = 1
            if const_expr(is_a8w4_gfx942):
                sorted_rows = size_expert_ids_in * arith.constant(tile_m, index=True)
                sx_nbytes_idx = sorted_rows * arith.constant(
                    scale_kblk_padded, index=True
                )
                sx_rsrc = ptr_buffer_resource(
                    arg_scale_x, arith.index_cast(T.i32, sx_nbytes_idx)
                )
            ...
            def _moe_gemm2_then_body():
                ...
                if const_expr(is_a16w4):
                    ...
                    _a16_x_elem = (
                        default_f8_type() if is_a8w4_gfx942 else T.bf16
                    )
                    w_elem = T.i8
                    ...
                    num_x_loads = bytes_per_thread_x // x_load_bytes
                    chunk_i32 = x_load_bytes // 4
                    _a16_vec16_x = T.vec(16 // elem_bytes, _a16_x_elem)
                    _a16_x_vec_ty = T.vec(
                        x_load_bytes // elem_bytes, _a16_x_elem
                    )
                    ...
                    def load_x_tile(base_k):
                        ...
                        for i in range_constexpr(num_x_loads):
                            ...
                            # idx_i32 is a dword index. The shared helper expects
                            # an element index and divides by two only for BF16,
                            # so multiplying by elem_bytes yields the same dword
                            # index for both BF16 and FP8.
                            idx_elem = idx_i32 * arith.index(int(elem_bytes))
                            x_vec = buffer_copy_gmem16_dwordx4(
                                buffer_ops,
                                vector,
                                elem_type=_a16_x_elem,
                                idx_i32=idx_elem,
                                rsrc=x_rsrc,
                                vec_elems=x_load_bytes // elem_bytes,
                                elem_bytes=elem_bytes,
                            )
                            parts.append(
                                vector.bitcast(T.vec(chunk_i32, i32), x_vec)
                            )
                        return parts
                    ...
                    def _sorted_scale_byte_index(sorted_row, scale_group):
                        """Mirror ``mx_scale_shuffle_idx`` for sorted A scales."""
                        scale_n_pad = arith.index(scale_kblk_padded)
                        return (
                            (sorted_row // arith.index(32) * scale_n_pad)
                            * arith.index(32)
                            + (scale_group // arith.index(8)) * arith.index(256)
                    ...
                    if const_expr(total_tiles == 1):
                        ...
                        if const_expr(_pad_ku_skip > 0):
                            ...
                            a_sc_tail = load_a_scales(
                                k_tail1, ku_limit=_tail_ku
                            )
                        else:
                            sc_raw_tail, kps_tail = load_b_scale_raw(k_tail1)
                            b_v4_tail = load_b_raw(k_tail1)
                            a_sc_tail = load_a_scales(k_tail1)
