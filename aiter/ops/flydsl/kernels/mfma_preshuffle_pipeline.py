...
def unpack_b_mxfp4_fp8(packed32, arith, vector):
    """Unpack 8 E2M1 nibbles into one gfx942 FNUZ FP8 MFMA operand.

    The checkpoint payload remains packed FP4.  Only the eight values consumed
    by the current K32 MFMA are expanded, in VGPRs, to their exact
    ``float8_e4m3fnuz`` byte encodings.  No FP8 weight tensor is materialized.
    """
    ...
    c0f_bytes = arith.constant(0x0F0F0F0F, type=T.i32)
    even = arith.andi(packed32, c0f_bytes)
    odd = arith.andi(
        arith.shrui(packed32, arith.constant(4, type=T.i32)), c0f_bytes
    )

    def permute_nibbles(selector):
        result = llvm_dialect.inline_asm(
            i32_ty,
            # ``v_perm_b32`` selector bytes 0..3 address the second source,
            # while 4..7 address the first.  Put odd nibbles in src0 and even
            # nibbles in src1 so 0x05010400 expands to n0,n1,n2,n3.
            [raw(odd), raw(even), raw(arith.constant(selector, type=T.i32))],
            "v_perm_b32 $0, $1, $2, $3",
            "=v,v,v,v",
    ...
    # Expand eight packed nibbles into two dwords of four byte-sized nibbles:
    # [n0,n1,n2,n3] and [n4,n5,n6,n7].
    nibble_words = (
        permute_nibbles(0x05010400),
        permute_nibbles(0x07030602),
    )

    c_mag_mask = arith.constant(0x07070707, type=T.i32)
    c_sign_mask = arith.constant(0x08080808, type=T.i32)
    c_byte_lsb = arith.constant(0x01010101, type=T.i32)
    ...
    c_56 = arith.constant(56, type=T.i32)

    def e2m1_bytes_to_fp8(nibbles):
        magnitude = arith.andi(nibbles, c_mag_mask)
        mag_or = arith.ori(
            magnitude,
            arith.ori(
                arith.shrui(magnitude, c_one),
                arith.shrui(magnitude, c_two),
            ),
        )
        nonzero = arith.andi(mag_or, c_byte_lsb)
        upper_bits = arith.andi(
            arith.ori(
                arith.shrui(magnitude, c_one),
                arith.shrui(magnitude, c_two),
            ),
        ...
        is_one = arith.andi(
            arith.andi(magnitude, c_byte_lsb),
            arith.xori(upper_bits, c_byte_lsb),
        )

        # Exact per-byte positive mapping:
        #   0,.5,1,1.5,2,3,4,6 -> 00,38,40,44,48,4c,50,54.
        positive = arith.subi(
            arith.addi(
                arith.shli(magnitude, c_two),
                arith.muli(nonzero, c_56),
            ),
            arith.muli(is_one, c_four),
        )
        # FNUZ reserves 0x80 for NaN, so E2M1 negative zero must canonicalize
        # to 0x00 rather than carrying its sign bit into the FP8 byte.
        sign = arith.andi(
            arith.shli(arith.andi(nibbles, c_sign_mask), c_four),
            arith.shli(nonzero, c_seven),
        )
        return arith.ori(positive, sign)

    words = [e2m1_bytes_to_fp8(nibbles) for nibbles in nibble_words]

    pair = vector.from_elements(T.i32x2, words)
    packed_fp8 = vector.bitcast(T.vec(1, T.i64), pair)
    return vector.extract(packed_fp8, static_position=[0], dynamic_position=[])
