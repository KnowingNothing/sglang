...
class AITER_CONFIG:
    ...
    def update_config_files(self, file_path: str, merge_name: str):
        ...
        # Shared images can contain a root-owned /tmp/aiter_configs while the
        # serving process runs as an unprivileged uid.  Keep one merge cache
        # per uid by default so the generated CSV and lock are writable and
        # cannot collide across container users.  The location remains
        # explicitly overridable for deployments with a persistent cache.
        config_path = Path(
            os.getenv("AITER_CONFIG_CACHE_DIR", f"/tmp/aiter_configs-{os.getuid()}")
        )
